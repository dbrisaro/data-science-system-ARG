...
datapath = '/home/daniufundar/Documents/Papers/data-science-system-ARG/datos/informacion_completa_csv/'
figspath = '/home/daniufundar/Documents/Papers/data-science-system-ARG/figs/periodo_2011-2021/'
tablespath = '/home/daniufundar/Documents/Papers/data-science-system-ARG/tablas/'
...
# In[3]:
## % publicaciones por genero y año

anios = pd.date_range('01-2011', '01-2021', freq='AS')
...
for i, iyear in enumerate(range(2011,2022)):
...
# In[4]:
## % publicaciones por genero, año y edad
...
# In[5]:
## % publicaciones por genero, año y seniority level
...
# In[6]:
## % publicaciones por genero, año y categoria conicet
...
anios = pd.date_range('01-2011', '01-2022', freq='AS')
...
# In[10]:
## Grafico - Publicaciones por genero
...
# In[12]:
## Grafico - Publicaciones por genero y edad
...
ax = plt.axes([0.1, 0.1, 0.85, 0.85])
gm = ax.bar(x-0.2, ym, width=0.4, color=color_m, alpha=1, yerr=lista_m_std, label='Femenino')
gh = ax.bar(x+0.2, yh, width=0.4, color=color_h, alpha=1, yerr=lista_h_std, label='Masculino')
ax.tick_params('both', labelsize=fontsize)
...
for i, iyear in enumerate(range(2011,2022)):
    ...
    if i==0:
        gm = ax.plot(x-0.1, valores_m, color=color_m, alpha=1, lw=0, marker='.', label='Mujeres')
        gh = ax.plot(x+0.1, valores_h, color=color_h, alpha=1, lw=0, marker='.', label='Hombres')
    else:
        gm = ax.plot(x-0.1, valores_m, color=color_m, alpha=1, lw=0, marker='.')
        gh = ax.plot(x+0.1, valores_h, color=color_h, alpha=1, lw=0, marker='.')
...
for j, jseniority in enumerate(xlabels):
    ...
    gm = ax.bar(x[j]-0.2, ym, width=0.4, color=color_m, alpha=1, label='Femenino')
    gh = ax.bar(x[j]+0.2, yh, width=0.4, color=color_h, alpha=1, label='Masculino')
    if j==0:
        ax.legend(fontsize=fontsize)
...
fig = plt.figure(figsize=(7,4))
ax = plt.axes([0.1, 0.1, 0.85, 0.85])
gm = ax.bar(x-0.2, ym, width=0.4, color=color_m, alpha=1, label='Mujeres')
gh = ax.bar(x+0.2, yh, width=0.4, color=color_h, alpha=1, label='Hombres')
ax.tick_params('both', labelsize=fontsize)
# ax.set_title('1)', fontsize=fontsize, loc='left')
ax.legend(fontsize=fontsize)
ax.set_xticks(x)
