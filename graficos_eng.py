...
# Figura 1
figname = 'evolucion_cantidad_publicaciones_por_genero_eng'
fontsize = 8
...
gm = ax.bar(x-0.2, ym, width=0.4, color=color_m, alpha=.8, label='Women')
gh = ax.bar(x+0.2, yh, width=0.4, color=color_h, alpha=.5, label='Men')
...
ax.set_ylabel('Papers per capita', fontsize=fontsize)

tpubl = df_publ_por_genero["Total publicaciones (N)"]
bx = ax.twinx()
bx.plot(x, tpubl, '*-', lw=0.5, color='blue')
bx.tick_params(labelsize=fontsize)
bx.set_ylabel('Total number of papers', fontsize=fontsize, color='blue')
...
# Figura 2
figname = 'cantidad_publicaciones_por_genero_y_edad_eng'
fontsize = 8
...
xlabels = ['21 a 30', '31 a 40', '41 a 50', '51 a 60', '>61']

fig = plt.figure(figsize=(6,4))
ax = plt.axes([0.1, 0.1, 0.85, 0.85])
gm = ax.bar(x-0.2, ym, width=0.4, color=color_m, alpha=.8, yerr=lista_m_std, label='Women')
gh = ax.bar(x+0.2, yh, width=0.4, color=color_h, alpha=.5, yerr=lista_h_std, label='Men')
ax.tick_params('both', labelsize=fontsize)
...
# Figura 4
figname = 'cantidad_publicaciones_por_genero_y_categoria_conicet_eng'
fontsize = 8
...
for j, jseniority in enumerate(xlabels):
    ...
    gm = ax.bar(x[j]-0.2, ym, width=0.4, color=color_m, alpha=.8, yerr = ym_std, label='Women')
    gh = ax.bar(x[j]+0.2, yh, width=0.4, color=color_h, alpha=.5, yerr = yh_std, label='Men')
    if j==0:
        ax.legend(fontsize=fontsize, loc='upper left')
...
ax.set_xlabel('Seniority level', fontsize=fontsize)
ax.set_ylabel('Papers per capita', fontsize=fontsize)
...
figname = 'Tipo_publicaciones_por_genero_eng'
fontsize = 8
x = np.arange(0, 7)
categ_publ=['Q1', 'Q2', 'Q3', 'Q4', 'NBR', 'Book chapters', 'Books']
xlabels = categ_publ
...
ax.set_ylim([0, 1.25])
ax.set_xlabel('Type of publication', fontsize=fontsize)
ax.set_ylabel('Papers per capita', fontsize=fontsize)
ax.grid(axis='y', lw=0.5, linestyle=':', color='dimgrey')
fig.savefig(figspath + figname + '.png', dpi=300, bbox_inches='tight')
fig.savefig(figspath + figname + '.pdf', bbox_inches='tight')
...
figname = 'Patentes_por_genero_eng'
fontsize = 8
x = np.arange(0, 2)
xlabels = categ_publ
